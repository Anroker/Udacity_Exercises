...
public class CandyDAOImpl implements CandyDAO {
    ...
    private static final String FIND_CANDY_BY_DELIVERY = "SELECT c.* FROM candy_delivery AS cd " + "JOIN candy AS c on c.id = cd.candy_id " + "WHERE cd.delivery_id = :" + DELIVERY_ID;

    private static final RowMapper<Candy> CANDY_ROW_MAPPER = new BeanPropertyRowMapper<>(Candy.class);

    final NamedParameterJdbcTemplate jdbcTemplate;

    public CandyDAOImpl(NamedParameterJdbcTemplate jdbcTemplate) {
    ...
    public List<Candy> list() {
        ...
        return jdbcTemplate.query(SELECT_ALL_CANDY, CANDY_ROW_MAPPER);
    }
    ...
    public List<Candy> findByDelivery(Long deliveryId) {
        return jdbcTemplate.query(FIND_CANDY_BY_DELIVERY,
                new MapSqlParameterSource(DELIVERY_ID, deliveryId),CANDY_ROW_MAPPER);
    }
}
